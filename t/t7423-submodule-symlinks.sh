...
test_expect_success SYMLINKS 'git checkout --recurse-submodules -- <pathspec> must not be confused by a symlink' '
	prepare_symlink_to_repo &&
	test_must_fail git checkout --recurse-submodules -- a/sm &&
	test_path_is_missing a/sm/submodule_file &&
	test_path_is_dir a/target/.git &&
	test_path_is_missing a/target/submodule_file
'

test_expect_failure SYMLINKS 'git checkout --recurse-submodules -- <pathspec> must not migrate git dir of symlinked repo' '
	prepare_symlink_to_repo &&
	rm -rf .git/modules &&
	test_must_fail git checkout --recurse-submodules -- a/sm &&
	test_path_is_dir a/target/.git &&
